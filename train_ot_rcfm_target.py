    ...
    def forward(self, x=None, y=None, cond=None, mode="train", patch_labels=None):
        if mode == "train":
            ...
            if patch_labels is not None:
                ...
                # 加权合并
                alpha = 0.9  # ← 你可以自由设置，例如 0.7 表示更关注 mask 区域
...
def plot_ecg_traces(x, y, xt, save_path=None):
    ...
    plt.plot(x[0], label="x0 (data)", alpha=0.8)
    plt.plot(y[0], label="x1 (noise)", alpha=0.8)
    plt.plot(xt[0], label="x_t (interpolated)", linestyle="--")
    plt.legend()
    plt.title("Flow Matching Trajectory Visualization")
    if save_path:
...
def train_flowmatching(config):
    ...
    for i in range(n_epoch):
        ...
        if (i+1) % 20 == 0:
            torch.save(flow_model.state_dict(), f"{PATH}/rcfm_target09_{flow_matcher_type}_epoch{i}.pth")
            torch.save(condition_net.state_dict(), f"{PATH}/condition_rcfm_target09_{flow_matcher_type}_epoch{i}.pth")
...
if __name__ == "__main__":
    ...
    config = {
        ...
        "PATH": "./saved/",
        "warmup_epochs": 10,
        "flow_matcher_type": "target",  # 可选: conditional / target / sb / vp
