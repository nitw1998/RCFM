...
class DDIM(nn.Module):
    def __init__(self, eps_model, betas, n_T, eta=0.0, criterion=nn.MSELoss()):
    ...
    def forward(self, x=None, cond=None, mode="train", window_size=128*4):
        if mode == "train":
            ...
            for i in reversed(range(1, self.n_T)):
                t = torch.full((n_sample,), i / self.n_T, device=device)
                alpha = self.alphabar_t[i]
                alpha_prev = self.alphabar_t[i - 1] if i > 1 else torch.tensor(1.0).to(device)

                eps = self.eps_model(x, cond, t)
                x0 = (x - self.sqrtmab[i] * eps) / self.sqrtab[i]

                sigma = self.eta * torch.sqrt((1 - alpha_prev) / (1 - alpha) * (1 - alpha / alpha_prev))
...
class SimpleFlowMatching(nn.Module):
    ...
    def forward(self, x=None, y=None, cond=None, mode="train"):
        if mode == "train":
            t = torch.rand(x.size(0), 1, 1).to(x.device)
            x_t = t * x + (1 - t) * y
            v_target = x - y
            v_pred = self.flow_model(x_t, cond, t.squeeze())
...
class RegionAwareFlowMatching(nn.Module):
    ...
    def create_masked_velocity(self, x, y, patch_labels):
        """
        Applies mask to velocity vector v = x - y, using patch_labels as ROI indicator.
        """
        v_target = x - y
    ...
    def forward(self, x=None, y=None, cond=None, mode="train", patch_labels=None, window_size=128*4):
        if mode == "train":
            t = torch.rand(x.size(0), 1, 1).to(x.device)
            x_t = t * x + (1 - t) * y

            v_target, mask = self.create_masked_velocity(x, y, patch_labels)

            v_pred = self.flow_model(x_t, cond, t.squeeze())

            masked_v_pred = v_pred * mask

            return self.criterion(masked_v_pred, v_target)
...
def load_pretrained_DPM(PATH, nT, type="RDDM", device="cuda"):
    ...
    if type == "RDDM":
        ...
        dpm.load_state_dict(torch.load(PATH + "rddm_main_network_epoch779.pth"))

        dpm = freeze_model(dpm)

        Conditioning_network1 = ConditionNet().to(device)
        Conditioning_network1.load_state_dict(torch.load(PATH + "ConditionNet1_epoch779.pth"))
        Conditioning_network1 = freeze_model(Conditioning_network1)

        Conditioning_network2 = ConditionNet().to(device)
        Conditioning_network2.load_state_dict(torch.load(PATH + "ConditionNet2_epoch779.pth"))
        ...
        cfm = RegionAwareFlowMatching(
            flow_model=DiffusionUNetCrossAttention(512, 1, device),
        ).to(device)

        cfm.load_state_dict(torch.load(PATH + f"rcfm_vp01_vp_epoch979.pth"))
        cfm = freeze_model(cfm)

        Conditioning_network = ConditionNet().to(device)
        Conditioning_network.load_state_dict(torch.load(PATH + f"condition_rcfm_vp01_vp_epoch979.pth"))
        Conditioning_network = freeze_model(Conditioning_network)

        return cfm, Conditioning_network, None
    
    elif type == "CFM":

        cfm = MinimalFlowMatching(
            flow_model=DiffusionUNetCrossAttention(512, 1, device),
            criterion=nn.MSELoss()
        ).to(device)

        cfm.load_state_dict(torch.load(PATH + f"minimal_cfm_epoch_779.pth"))
        cfm = freeze_model(cfm)
        Conditioning_network = ConditionNet().to(device)
        Conditioning_network.load_state_dict(torch.load(PATH + f"condition_net_epoch_779.pth"))
        Conditioning_network = freeze_model(Conditioning_network)
        ...
        dpm.load_state_dict(torch.load(PATH + f"ddim_main_network_{nT}.pth"))
        dpm = freeze_model(dpm)

        Conditioning_network = ConditionNet().to(device)
        Conditioning_network.load_state_dict(torch.load(PATH + f"ddim_condition_encoder_{nT}.pth"))
        Conditioning_network = freeze_model(Conditioning_network)
...
if __name__ == "__main__":
    ...
    # dummy_input = torch.randn(2, 1, 128*30).to(device)

    # dummy_conditions = ConditionNet().to(device)(torch.randn(2, 1, 128*30).to(device))

    # dummy_ts = torch.randint(0, 100, [2]).to(device)

    # torchinfo.summary(model, input_data=(dummy_input, dummy_conditions, dummy_ts), device=device)


    model = RegionAwareFlowMatching(
        flow_model=DiffusionUNetCrossAttention(512, 1, device=device)
    ).to(device)
    dummy_input = torch.randn(2, 1, 128*30).to(device)
    dummy_conditions = ConditionNet().to(device)(torch.randn(2, 1, 128*30).to(device))
    dummy_ts = torch.randint(0, 100, [2]).to(device)
    dummy_patch_labels = torch.randint(0, 2, [2, 1, 128*30]).to(device)     

    torchinfo.summary(model, input_data=(dummy_input, dummy_conditions, dummy_ts, dummy_patch_labels), device=device)