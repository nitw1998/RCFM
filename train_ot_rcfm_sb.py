    ...
    def forward(self, x=None, y=None, cond=None, mode="train", patch_labels=None):
        if mode == "train":
            ...
            if patch_labels is not None:
                ...
                # 加权合并
                alpha = 0.01  # 可以自由设置，例如 0.7 表示更关注 mask 区域
...
def train_flowmatching(config):
    ...
    for i in range(n_epoch):
        ...
        if (i+1) % 20 == 0:
            torch.save(flow_model.state_dict(), f"{PATH}/{dataset}/rcfm_sb001_{flow_matcher_type}_epoch{i}.pth")
            torch.save(condition_net.state_dict(), f"{PATH}/{dataset}/condition_rcfm_sb001_{flow_matcher_type}_epoch{i}.pth")
...
if __name__ == "__main__":
    ...
    config = {
        ...
        "PATH": "./saved/",
        "warmup_epochs": 10,
        "flow_matcher_type": "sb",  # 可选: conditional / target / sb / vp
