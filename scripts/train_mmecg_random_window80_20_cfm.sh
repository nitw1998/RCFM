...
DATA_ROOT="${MMECG_RANDOM_WINDOW_DATA_ROOT:?Set MMECG_RANDOM_WINDOW_DATA_ROOT to the directory containing mmECG/}"
RUN_ROOT="${RCFM_RUNS_ROOT:?Set RCFM_RUNS_ROOT to the training output root}"
CONFIG="configs/mmecg/cfm_random_window80_20_seed31.yaml"
MANIFEST="$DATA_ROOT/mmECG/dataset_manifest.json"
RUN_ID="mmecg_cfm_random_window80_20_s31_e200_v1"
RUN_DIR="$RUN_ROOT/rcg2ecg/mmECG/$RUN_ID"
...
for path in \
  "$MANIFEST" \
  "$DATA_ROOT/mmECG/ppg_train_4sec.npy" \
  "$DATA_ROOT/mmECG/ecg_train_4sec.npy" \
  "$DATA_ROOT/mmECG/ppg_test_4sec.npy" \
  "$DATA_ROOT/mmECG/ecg_test_4sec.npy" \
  "$DATA_ROOT/mmECG/subject_ids_train.npy" \
  "$DATA_ROOT/mmECG/source_files_train.npy" \
  "$REPO_ROOT/$CONFIG"; do
  if [[ ! -f "$path" ]]; then
...
assert m["dataset_version"] == "mmecg-all-windows-random80-20-subject-record-overlap-window-minmax-v1"
assert m["split_hash"] == "6e5365be9b71c3815907eeabab2ee6b83a11a280521243a1f79c4f90da570dc2"
assert m["splits"]["train"]["windows"] == 9973
assert m["splits"]["test"]["windows"] == 2494
assert m["overlap"]["subjects_in_both_train_and_test"] == 11
assert m["overlap"]["source_records_in_both_train_and_test"] == 91
assert m["overlap"]["cross_split_adjacent_window_pairs"] == 3977
...
PID=$!
echo "$PID" >"$PID_PATH"
echo "Started mmECG random-window 80:20 CFM, 200 epochs."
echo "Run ID: $RUN_ID"
echo "PID: $PID"
echo "Log: $LOG_PATH"
